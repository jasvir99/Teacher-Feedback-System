\documentclass[12pt]{article}
\usepackage[print,nopanel]{pdfscreen}
\begin{print}
\usepackage{lastpage}
\usepackage[colorlinks]{hyperref}

\usepackage{cleveref}
\usepackage{fancyhdr}
\usepackage{verbatim}
\lhead{\large\bfseries TFS}
\usepackage[left=2.5cm, right=1.5cm, top=2.5cm, bottom=1.5cm]{geometry}
\pagestyle{fancy}
\end{print}
\margins{.5cm}{.5cm}{.5cm}{.5cm}
\begin{screen}

\renewcommand{\encodingdefault}{T1}
\usepackage{setspace}
\linespread{1.5}
\renewcommand{\rmdefault}{ptm}
\end{screen}
\screensize{8cm}{9cm}
\overlay{overlay8.pdf}
\usepackage{graphicx}

\begin{document}
\input{input/titlepage.tex}
\begin{screen}
\ppttitle
\end{screen}
\footskip 0.7cm
\thispagestyle{empty} 
\pagetitle
\newpage
\pagenumbering{Roman}
\cfoot{\thepage}
\newpage
\tableofcontents
\newpage
\pagenumbering{arabic}
\cfoot{\thepage}
\newpage
\rhead{Introduction}
\section{Introduction}
\input{input/introduction.tex}
\newpage
\rhead{The General Description}
\section{The General Description}
\input{input/general_description.tex}
\newpage
\rhead{Specific Requirements}
\section{Specific Requirements}
\input{input/specific_requirements.tex}
\end{document}

